...
if (isset($_POST['login'])) {
    ...
    $sql = "SELECT * FROM `therapistregistration` WHERE `phone` = '$phone' AND `password` = '$password' ORDER BY id DESC LIMIT 1";
    $sql_result = mysqli_query($naturopathyCon, $sql);
    $sql_result_array = mysqli_fetch_array($sql_result);
    if (isset($sql_result_array['id'])) {
        $_SESSION['therapistFullName'] = $sql_result_array['fullName'];
        $_SESSION['therapistUserId'] = $sql_result_array['id'];
        $_SESSION['therapistSessionActive'] = True;
        header('location: dashboard.php');
    } else {
        $error = TRUE;
    }
...
<head>
    ...
    <title>Aarogyadham-Naturopathy-Yoga-Center | Therapist Login</title>
