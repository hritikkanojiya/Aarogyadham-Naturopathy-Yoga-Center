...
if (isset($_SESSION['patientSessionActive'])) {
    ...
    if (isset($_POST['resetPass'])) {
        $cpass = $_POST['cpass'];
        $npass = $_POST['npass'];
        if ($cpass == $activePatientDataResult['password']) {
            $updatePass = mysqli_query($naturopathyCon, "UPDATE `patientregistration` SET `password` = '$npass' WHERE `patientregistration`.`id` = $patientUserId");
